...
export function CallCenterBanner({ embedded = false }: { embedded?: boolean }) {
  ...
  return (
    <section className={embedded ? "absolute bottom-0 inset-x-0 z-20" : "relative z-10 -mt-1"}>
      <div className="bg-gradient-to-r from-navy-900 via-navy-800 to-kz-blue-dark">
        ...
        <div
          className={`relative max-w-7xl mx-auto px-5 lg:px-8 ${
            embedded ? "py-2.5 sm:py-3" : "py-4 sm:py-5"
          }`}
        >
          <div
            className={`flex flex-col lg:flex-row lg:items-center lg:justify-between ${
              embedded ? "gap-2 sm:gap-3" : "gap-5"
            }`}
          >
            <div className={`flex items-center shrink-0 ${embedded ? "gap-2.5" : "gap-4"}`}>
              <div
              ...
              >
                <Phone className={embedded ? "w-4 h-4 sm:w-[18px] sm:h-[18px] text-kz-gold" : "w-6 h-6 sm:w-7 sm:h-7 text-kz-gold"} />
              </div>
              <div className="min-w-0">
                ...
                <h2
                  className={`font-bold text-white leading-tight ${
                    embedded
                      ? "text-[12px] sm:text-[13px] lg:text-sm line-clamp-2"
            ...
            <div
              className={`flex flex-wrap items-center ${
                embedded ? "gap-x-3 gap-y-1 sm:gap-x-4" : "gap-4 sm:gap-6 lg:gap-8"
              }`}
            >
              {features.map(({ icon: Icon, key }) => (
                <div key={key} className="flex items-center gap-1.5 text-white/85">
                  <Icon className={`text-kz-blue-light shrink-0 ${embedded ? "w-3 h-3" : "w-4 h-4"}`} />
                  <span
                    className={`font-medium whitespace-nowrap ${
                      embedded ? "text-[10px] sm:text-[11px]" : "text-[12px] sm:text-[13px]"
                    }`}
                  >
            ...
            <a
              ...
              className={`inline-flex items-center justify-center bg-kz-gold text-navy-900 font-bold shadow-lg shadow-kz-gold/25 hover:bg-[#f5b800] transition-colors shrink-0 ${
                embedded
                  ? "gap-1.5 px-3.5 py-2 rounded-lg text-[11px] sm:text-xs"
